...
def send_welcome(message):
  ...
  if not is_member_of_channel(user_id):
    ...
    join = types.InlineKeyboardButton("join", url=f"https://t.me/Nepsxbot")
  ...
  button1 = KeyboardButton("🛒reaction")
  button6 = KeyboardButton("🛒auto reaction")
  button7 = KeyboardButton("💔 cancel autoreaction")
  button8 = KeyboardButton("views")
  ...
  bot.send_photo(user_id, logo_url, caption="""Hi, welcome to reaction bot ✋\n\n🤗With reaction bot  it's just a few taps to increase number of reactions of your Telegram posts.💓\n\n
...
"""
  ...
  if message.text == "🗣 reffersssssijh":
...
Yes, if your balance is more than 5k and you want to transfer all of them, you can send a request to support @Aviii56 here you can request coin transfer.
🆘 In case you have any problem, contact @Aviii56"""
    bot.reply_to(message, msg, parse_mode="html")
...
<b><u>📜 Packages:</u></b>
<b> 🪙1k coins for 8 India rupees
🪙 10k coins for 80 inr
🪙 20k views for 160inr
</b>

💰 Pay with Bitcoin, USDT, BSC, BUSD,  ... 👉🏻 @Aviii56
available payment methods Paytm, phone pe, binance, ... 👉🏻 @Aviii56

<b>🆔 Your id:</b> <code>{user_id}</code>
"""
    markup = InlineKeyboardMarkup()
    button1 = InlineKeyboardButton("💲 binance", url="https://t.me/Aviii56")
    button2 = InlineKeyboardButton("gpay",
                                   url="https://t.me/Aviii56")
    button4 = InlineKeyboardButton("💸 Paytm", url="https://t.me/Aviii56")
    button5 = InlineKeyboardButton("💰 phone pe", url="https://t.me/Aviii56")
  ...
  if message.text == "🛒reaction":
    ...
    markup.add(button1)
    msg = f"""👉‍ Enter number of reactions in range ({min_view}, {max_view}) 👇🏻
...
def view_amount(message):
  ...
  if int(amount) < min_view:
    bot.send_message(user_id,
                     f"❌ Minimum - {min_view} reactions",
                     parse_mode="Markdown",
                     reply_markup=markup)
    return
  if float(amount) > float(bal):
    bot.send_message(user_id,
                     "❌ You can't purchase more reactions than your balance",
                     parse_mode="Markdown",
                     reply_markup=markup)
    return
...
def view_link(message, amount):
  ...
  bot.send_message(user_id,
                   (f"*✅ Your Order Has Been Submitted and Processing\n\n"
                    ...
                    f"👀 *Tg Post reactions  :* `{amount} reactions`\n\n"
